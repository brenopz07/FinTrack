...
export default function ModalReceita({
  ...
  const [novaCategoriaId, setNovaCategoriaId] = useState(
    transacao?.category?.id || ""
  );
  const [novoTipo, setNovoTipo] = useState(transacao?.type || "income");
  ...
  useEffect(() => {
    if (transacao && categorias.length > 0) {
      const categoriaId =
        transacao.category_id ||
        transacao.category?.id ||
        categorias.find(cat => cat.name === transacao.categoria)?.id ||
        "";
      ...
      setNovoTipo(
        transacao.type === "income" || transacao.tipo === "receita"
          ? "income"
          : "expense"
      );
  ...
  const atualizar = async () => {
    try {
      ...
      if (typeof atualizarTransacoes === "function") {
      ...
      const listaAtualizada = receitas.map((item) => {
        if (item.id === transacao.id) {
          return {
            ...
            data: novaData,
            categoria:
              categorias.find((cat) => cat.id === novaCategoriaId)?.name ||
              transacao.categoria,
              tipo: novoTipo === "income" ? "receita" : "despesa",
          };
        }
        return item;
      });
          ...
          <CardModal dark={dark}>
            ...
            >
              ...
              <Categoria
                style={{
                  backgroundColor:
                    transacao.tipo === "despesa" ? "#EA43354D" : "#34A85326",
                }}
              >
                <Image
                  style={{ resizeMode: "contain", width: 12, height: 12 }}
                  source={transacao.tipo === "despesa" ? despesa1 : receita1}
                />
                {edit ? (
                  <Picker
                    selectedValue={novaCategoriaId}
                    onValueChange={(value) => setNovaCategoriaId(value)}
                    style={{
                      color:
                        transacao.tipo === "receita" ? "#34A853" : "#EA4335",
                      width: 160,
                    }}
                  >
                    {categorias.map((cat) => (
                  ...
                  <MiniTexto
                    style={{
                      width: "auto",
                      color:
                        transacao.tipo === "despesa" ? "#EA4335" : "#34A853",
                    }}
                  >
                    {transacao.categoria}
                  </MiniTexto>
              ...
              {edit ? (
                <View>
                  <TextoInput
                    ...
                    blurOnSubmit={true}
                  />
                  <MiniTexto dark={dark}>Tipo</MiniTexto>
  ...
  <Picker
    ...
    style={{
      color: novoTipo === "income" ? "#34A853" : "#EA4335",
    }}
  >
    ...
    <Picker.Item label="Despesa" value="expense" />
            ...
            <View style={{ flexDirection: "row", gap: 16, width: "100%" }}>
              ...
              >
                ...
                ></Image>
                <Texto style={{ alignSelf: "center", color: "#EA4335" }}>
                  Apagar
                </Texto>
              </BotaoApagar>
              <BotaoEdit
              ...
              >
                <Image
                  ...
                  style={{ width: 12, height: 13 }}
                ></Image>
                <Texto dark={dark} style={{ alignSelf: "center" }}>
                  Editar
                </Texto>
              </BotaoEdit>
            </View>
